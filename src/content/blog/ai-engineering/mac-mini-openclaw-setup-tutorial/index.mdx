---
title: '실전! 맥미니(Mac mini)를 최강의 AI 에이전트 서버로 만드는 법 (feat. OpenClaw)'
date: '2026-02-11'
...
지난 글에서 왜 OpenClaw를 도입했는지, 그리고 어떤 멀티 에이전트 팀을 꾸렸는지 소개했다. 이번 글에서는 약속대로 내 책상 위에서 24시간 잠들지 않고 일하는 맥미니(Mac mini)를 최강의 AI 에이전트 서버로 세팅하는 과정을 튜토리얼 형태로 공유한다.
...
3. Unix 기반 환경: 대부분의 AI 도구와 라이브러리는 리눅스/맥 환경에서 가장 안정적이다.
...
이제 오늘의 주인공, OpenClaw를 설치할 차례다. 전역(Global)으로 설치하여 어디서든 CLI 명령어를 사용할 수 있게 한다.
...
3. 봇 토큰(Token)을 잘 보관해두자.

## 4단계: 게이트웨이 설정 (The openclaw.json)
...
## 6단계: 외부 접근 (Tailscale의 마법)

집 밖에서도 에이전트와 대화하고 싶다면 Tailscale을 추천한다. 복잡한 포트 포워딩 없이도 안전하게 맥미니 노드에 접근할 수 있다. OpenClaw 설정에서 gateway.tailscale.mode: "on"을 켜주기만 하면 끝이다.
